...
def remove_cycles(path_list):
    ...
    for i, path in enumerate(path_list):
        if path.last in path.route[:-1]:
            listIndex.append(i)
    return [path for i, path in enumerate(path_list) if i not in listIndex]
...
def depth_first_search(origin_id, destination_id, map):
    ...
    while queue and queue[0].last != destination_id:
        ...
        E = expand(C, map)
        E = remove_cycles(E)
        queue = insert_depth_first_search(E, queue)
    if queue:
        return queue[0]
    else:
...
def breadth_first_search(origin_id, destination_id, map):
    ...
    while queue and queue[0].last != destination_id:
        ...
        E = expand(C, map)
        E = remove_cycles(E)
        queue = insert_breadth_first_search(E, queue)
    if queue:
        return queue[0]
    else:
        return "Non-Existent Path"
